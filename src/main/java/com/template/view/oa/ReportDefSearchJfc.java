...
public class ReportDefSearchJfc {
    private static Logger LOG = Logger.getLogger(ReportDefSearchJfc.class.getName());
    
    private ReportDefSearchModel model;
    private JDialog dialog;
    protected ReportDefJfc jfcReportDef;
    
    protected boolean bShowResults;
    protected boolean bMultiSelect;
    protected ReportDef reportDefSelected;
    protected boolean bSelected;
    
    public ReportDefSearchJfc() {
        this(null, true, false);
    }
    public ReportDefSearchJfc(ReportDefSearchModel model) {
        this(model, true, false);
    }
    public ReportDefSearchJfc(ReportDefSearchModel model, boolean bShowResults, boolean bMultiSelect) {
        this.model = model;
        this.bShowResults = bShowResults;
        this.bMultiSelect = bMultiSelect;
    }
    public ReportDefSearchJfc(boolean bShowResults) {
        this(null, bShowResults, false);
    }
    
    public ReportDefSearchModel getModel() {
        if (model == null) {
            model = new ReportDefSearchModel();
            model.getReportDefSearch().setMaxResults(100); // default for UI searches
        }
        return model;
    }
    ...
    public JPanel createMainPanel() {
        ...
        if (cnt > 1) {
            ...
            Icon icon = Resource.getJarIcon("reportDef.gif");
            if (panSearch != null) {
    ...
    public JPanel createSearchFromPanel() {
        final Hub<ReportDef> hub = getModel().getSearchFromHub();
        ...
        if (bMultiSelect) table.setMultiSelectControlKey(true);
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    ReportDef reportDef = hub.getAO();
                    if (reportDef != null) onSelect(reportDef, hub);
                }
            });
        }
        return pan;
    ...
    public JPanel createMultiSelectPanel() {
        final Hub<ReportDef> hub = getModel().getMultiSelectHub();
        JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        JLabel lbl = new JLabel("ReportDefs to add");
    ...
    public JPanel createSearchTreePanel() {
        // no tree properties defined in model, for ReportDef
        if (true || false) return null;
        // if (!bShowResults) return null;
        final JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        final ReportDefModel model = new ReportDefModel();
        if (bMultiSelect) {
        ...
        if (!bMultiSelect) {
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    ReportDef reportDef = model.getHub().getAO(); 
                    if (reportDef != null) onSelect(reportDef, model.getHub());
                }
            });
            pan.add(panx, BorderLayout.SOUTH);
        }
        ...
        SwingWorker<OATree, Void> sw = new SwingWorker<OATree, Void>() {
            ...
            protected OATree doInBackground() throws Exception {
                ReportDefJfc jfcReportDef = new ReportDefJfc(model);
                OATree tree = jfcReportDef.createSearchTree();
                tree.preload();
                return tree;
            }
            @Override
    ...
    protected JPanel createSearchResultsPanel() {
        ...
        final Hub<ReportDef> hub = getHub();
        OATable table = createReadOnlyTable(hub);
        OATableScrollPane spTable = new OATableScrollPane(table, 1);
    ...
    public OATextField createCustomQueryTextField() {
        OATextField txt = new OATextField(getModel().getReportDefSearchHub(), ReportDefSearch.P_CustomQuery, 25);
        txt.setMinimumColumns(10);
        txt.setMaximumColumns(110);
        new OATextFieldCodeController(txt, ReportDef.class, OATextFieldCodeController.Type.query);
        return txt;
    }
    
    public OATextField createMaxResultsTextField() {
        OATextField txt = new OATextField(getModel().getReportDefSearchHub(), ReportDefSearch.P_MaxResults, 5);
    ...
    public OATable createReadOnlyTable(final Hub<ReportDef> hubReportDef) {
        Hub<ReportDef> hubx = new Hub(ReportDef.class);
        OATable table = new OATable(hubx) {
            @Override
            public void onDoubleClick() {
                if (ReportDefSearchJfc.this.getModel().getMultiSelectHub() != hubReportDef) {
                    ReportDef reportDef = hubReportDef.getAO();
                    if (reportDef != null) onSelect(reportDef, hubReportDef);
                }
            }
        };
        table.setFilterMasterHub(hubReportDef);
        table.setAllowDnD(false);
        table.setAllowSorting(true);
        table.addCounterColumn();
    
        if (bMultiSelect && getModel().getMultiSelectHub() != hubReportDef) {
    ...
    protected void createTableColumns(OATable table) {
        getReportDefJfc().createReadOnlyTableColumns(table);
    }
    public OAButton createSelectOneButton(final Hub<ReportDef> hub) {
        ...
        cmd.setToolTipText("Add selected ReportDef");
        OAButton.setup(cmd);
    ...
    public JButton createSelectManyButton() {
        ...
        cmd.setToolTipText("Add selected ReportDefs");
    ...
    public OAButton createRemoveButton(Hub<ReportDef> hub) {
        OAButton cmd = new OAButton(hub, OAButton.REMOVE);
        cmd.setText("Remove");
        cmd.setup();
        cmd.setToolTipText("Remove selected ReportDef");
        return cmd;
    }
    public OAButton createRemoveAllButton(Hub<ReportDef> hub) {
        ...
        cmd.setup();
        cmd.setToolTipText("Remove all ReportDefs from the select list");
        return cmd;
    }
    ...
    public ReportDefJfc getReportDefJfc() {
        if (jfcReportDef == null) {
            jfcReportDef = new ReportDefJfc(new Hub<ReportDef>(ReportDef.class));
            OAObjectModel om = jfcReportDef.getModel();
    ...
    public OAButton createGotoEditButton(final Hub<ReportDef> hub) {
        OAButton cmd = new OAButton(hub, "Details ...", Resource.getJarIcon(Resource.IMG_Goto), OAButton.GOTO) {
            @Override
            public void afterActionPerformed() {
                getReportDefJfc().getHub().setSharedHub(hub, true);
                getReportDefJfc().getEditDialog(this).setVisible(true);
    ...
    public JDialog getDialog() {
        ...
        dialog = new JDialog(win, "Report Def Search", ModalityType.APPLICATION_MODAL);
        ...
        dialog.addWindowListener(new WindowAdapter() {
            ...
            public void windowActivated(WindowEvent e) {
                ReportDefSearchJfc.this.bSelected = false;
                onShowDialog();
            }
            @Override
            public void windowClosing(WindowEvent e) {
                if (!ReportDefSearchJfc.this.bSelected) {
                    ReportDefSearchJfc.this.onCancel();
                }
                onHideDialog();
            }
        });
    ...
    protected void onReset() {
        getHub().cancelSelect();
        getHub().clear();
        getModel().getReportDefSearch().reset();
    }
    
    protected void onSelect(ReportDef reportDef, Hub<ReportDef> hubFrom) {
        reportDefSelected = reportDef;
        bSelected = true;
        getHub().cancelSelect();
        getModel().onSelect(reportDef, hubFrom);
        if (dialog != null) dialog.setVisible(false);
    }
    protected void onSelect(Hub<ReportDef> hub) {
        bSelected = true;
        getHub().cancelSelect();
    ...
    protected void onSearch(final JComponent compFrom) {
        /*
        if (!getModel().getReportDefSearch().isDataEntered()) {
            String msg = "Please enter selection data.";
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom), msg, "ReportDef Search", JOptionPane.WARNING_MESSAGE);
            return;
        }
        */
        ...
        dlgWait.setStatus("Selecting ReportDefs ... please wait ...");
        dlgWait.setTitle("ReportDef Search");
        ...
        if (dlgWait.wasCancelled()) {
            ...
            String msg = "No ReportDefs were found.";
            try {
                sw.get();
            }
            catch (Exception e) {
            ...
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                msg, "ReportDef Search",
                JOptionPane.WARNING_MESSAGE
            );
            if (compFocus != null) compFocus.requestFocus();
        }
        else {
            int max = getModel().getReportDefSearch().getMaxResults();
            if (max > 0 && getHub().getSize() == max) {
                String msg = "Selected the first (max="+max+") Report Defs.";
                JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                    msg, "ReportDef Search",
    ...
    public ReportDef getSelected() {
        return reportDefSelected;
    }
