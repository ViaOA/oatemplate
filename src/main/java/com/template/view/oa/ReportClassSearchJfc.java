...
public class ReportClassSearchJfc {
    private static Logger LOG = Logger.getLogger(ReportClassSearchJfc.class.getName());
    
    private ReportClassSearchModel model;
    private JDialog dialog;
    protected ReportClassJfc jfcReportClass;
    
    protected boolean bShowResults;
    protected boolean bMultiSelect;
    protected ReportClass reportClassSelected;
    protected boolean bSelected;
    
    public ReportClassSearchJfc() {
        this(null, true, false);
    }
    public ReportClassSearchJfc(ReportClassSearchModel model) {
        this(model, true, false);
    }
    public ReportClassSearchJfc(ReportClassSearchModel model, boolean bShowResults, boolean bMultiSelect) {
        this.model = model;
        this.bShowResults = bShowResults;
        this.bMultiSelect = bMultiSelect;
    }
    public ReportClassSearchJfc(boolean bShowResults) {
        this(null, bShowResults, false);
    }
    
    public ReportClassSearchModel getModel() {
        if (model == null) {
            model = new ReportClassSearchModel();
            model.getReportClassSearch().setMaxResults(100); // default for UI searches
        }
        return model;
    }
    ...
    public JPanel createMainPanel() {
        ...
        if (cnt > 1) {
            ...
            Icon icon = Resource.getJarIcon("reportClass.gif");
            if (panSearch != null) {
    ...
    public JPanel createSearchFromPanel() {
        final Hub<ReportClass> hub = getModel().getSearchFromHub();
        ...
        if (bMultiSelect) table.setMultiSelectControlKey(true);
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    ReportClass reportClass = hub.getAO();
                    if (reportClass != null) onSelect(reportClass, hub);
                }
            });
        }
        return pan;
    ...
    public JPanel createMultiSelectPanel() {
        final Hub<ReportClass> hub = getModel().getMultiSelectHub();
        JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        JLabel lbl = new JLabel("ReportClasses to add");
    ...
    public JPanel createSearchTreePanel() {
        // no tree properties defined in model, for ReportClass
        if (true || false) return null;
        // if (!bShowResults) return null;
        final JPanel pan = new JPanel(new BorderLayout(0,0));
        pan.setBorder(new EmptyBorder(5,5,5,5));
    
        final ReportClassModel model = new ReportClassModel(ModelDelegate.getReportClasses().createSharedHub());
        if (bMultiSelect) {
        ...
        if (!bMultiSelect) {
            ...
            pan.getActionMap().put(cmdName, new AbstractAction() {
                public void actionPerformed(ActionEvent e) {
                    ReportClass reportClass = model.getHub().getAO(); 
                    if (reportClass != null) onSelect(reportClass, model.getHub());
                }
            });
            pan.add(panx, BorderLayout.SOUTH);
        }
        ...
        SwingWorker<OATree, Void> sw = new SwingWorker<OATree, Void>() {
            ...
            protected OATree doInBackground() throws Exception {
                ReportClassJfc jfcReportClass = new ReportClassJfc(model);
                OATree tree = jfcReportClass.createSearchTree();
                tree.preload();
                return tree;
            }
            @Override
    ...
    protected JPanel createSearchResultsPanel() {
        ...
        final Hub<ReportClass> hub = getHub();
        OATable table = createReadOnlyTable(hub);
        OATableScrollPane spTable = new OATableScrollPane(table, 1);
    ...
    public OATextField createCustomQueryTextField() {
        OATextField txt = new OATextField(getModel().getReportClassSearchHub(), ReportClassSearch.P_CustomQuery, 25);
        txt.setMinimumColumns(10);
        txt.setMaximumColumns(110);
        new OATextFieldCodeController(txt, ReportClass.class, OATextFieldCodeController.Type.query);
        return txt;
    }
    
    public OATextField createMaxResultsTextField() {
        OATextField txt = new OATextField(getModel().getReportClassSearchHub(), ReportClassSearch.P_MaxResults, 5);
    ...
    public OATable createReadOnlyTable(final Hub<ReportClass> hubReportClass) {
        Hub<ReportClass> hubx = new Hub(ReportClass.class);
        OATable table = new OATable(hubx) {
            @Override
            public void onDoubleClick() {
                if (ReportClassSearchJfc.this.getModel().getMultiSelectHub() != hubReportClass) {
                    ReportClass reportClass = hubReportClass.getAO();
                    if (reportClass != null) onSelect(reportClass, hubReportClass);
                }
            }
        };
        table.setFilterMasterHub(hubReportClass);
        table.setAllowDnD(false);
        table.setAllowSorting(true);
        table.addCounterColumn();
    
        if (bMultiSelect && getModel().getMultiSelectHub() != hubReportClass) {
    ...
    protected void createTableColumns(OATable table) {
        getReportClassJfc().createReadOnlyTableColumns(table);
    }
    public OAButton createSelectOneButton(final Hub<ReportClass> hub) {
        ...
        cmd.setToolTipText("Add selected ReportClass");
        OAButton.setup(cmd);
    ...
    public JButton createSelectManyButton() {
        ...
        cmd.setToolTipText("Add selected ReportClasss");
    ...
    public OAButton createRemoveButton(Hub<ReportClass> hub) {
        OAButton cmd = new OAButton(hub, OAButton.REMOVE);
        cmd.setText("Remove");
        cmd.setup();
        cmd.setToolTipText("Remove selected ReportClass");
        return cmd;
    }
    public OAButton createRemoveAllButton(Hub<ReportClass> hub) {
        ...
        cmd.setup();
        cmd.setToolTipText("Remove all ReportClasss from the select list");
        return cmd;
    }
    ...
    public ReportClassJfc getReportClassJfc() {
        if (jfcReportClass == null) {
            jfcReportClass = new ReportClassJfc(new Hub<ReportClass>(ReportClass.class));
            OAObjectModel om = jfcReportClass.getModel();
    ...
    public OAButton createGotoEditButton(final Hub<ReportClass> hub) {
        OAButton cmd = new OAButton(hub, "Details ...", Resource.getJarIcon(Resource.IMG_Goto), OAButton.GOTO) {
            @Override
            public void afterActionPerformed() {
                getReportClassJfc().getHub().setSharedHub(hub, true);
                getReportClassJfc().getEditDialog(this).setVisible(true);
    ...
    public JDialog getDialog() {
        ...
        dialog = new JDialog(win, "Report Class Search", ModalityType.APPLICATION_MODAL);
        ...
        dialog.addWindowListener(new WindowAdapter() {
            ...
            public void windowActivated(WindowEvent e) {
                ReportClassSearchJfc.this.bSelected = false;
                onShowDialog();
            }
            @Override
            public void windowClosing(WindowEvent e) {
                if (!ReportClassSearchJfc.this.bSelected) {
                    ReportClassSearchJfc.this.onCancel();
                }
                onHideDialog();
            }
        });
    ...
    protected void onReset() {
        getHub().cancelSelect();
        getHub().clear();
        getModel().getReportClassSearch().reset();
    }
    
    protected void onSelect(ReportClass reportClass, Hub<ReportClass> hubFrom) {
        reportClassSelected = reportClass;
        bSelected = true;
        getHub().cancelSelect();
        getModel().onSelect(reportClass, hubFrom);
        if (dialog != null) dialog.setVisible(false);
    }
    protected void onSelect(Hub<ReportClass> hub) {
        bSelected = true;
        getHub().cancelSelect();
    ...
    protected void onSearch(final JComponent compFrom) {
        /*
        if (!getModel().getReportClassSearch().isDataEntered()) {
            String msg = "Please enter selection data.";
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom), msg, "ReportClass Search", JOptionPane.WARNING_MESSAGE);
            return;
        }
        */
        ...
        dlgWait.setStatus("Selecting ReportClasss ... please wait ...");
        dlgWait.setTitle("ReportClass Search");
        ...
        if (dlgWait.wasCancelled()) {
            ...
            String msg = "No ReportClasses were found.";
            try {
                sw.get();
            }
            catch (Exception e) {
            ...
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                msg, "ReportClass Search",
                JOptionPane.WARNING_MESSAGE
            );
            if (compFocus != null) compFocus.requestFocus();
        }
        else {
            int max = getModel().getReportClassSearch().getMaxResults();
            if (max > 0 && getHub().getSize() == max) {
                String msg = "Selected the first (max="+max+") Report Classes.";
                JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(compFrom),
                    msg, "ReportClass Search",
    ...
    public ReportClass getSelected() {
        return reportClassSelected;
    }
