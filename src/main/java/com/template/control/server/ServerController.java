...
public abstract class ServerController {
	...
	public boolean start() throws Exception {
		try {
			...
			if (controlServerFrame != null) controlServerFrame.setProcessing(false);
		}
	}
	...
	private boolean _start() throws Exception {
		...
		if (bRunAsService) LOG.config("Running as a Windows Service");
        ...
        for (int i=0; bCheckForDatasourceCorruption && i<2; i++) {
            ...
            if (!getDataSourceController().isDatabaseCorrupted()) break;
        
            String msg = "DataSource is not valid.";
            LOG.severe(msg);  // this will cause program to exit
            if (i > 0) throw new Exception(msg);
	...
	protected void createRemoteServerController() throws Exception {
		...
		controlRemote.getSyncServer().createLookup(RemoteSpellCheckInterface.BindName, getRemoteSpellCheck(), RemoteSpellCheckInterface.class);
    ...
    protected void handleException(int clientId, boolean bFromServer, String msg, Throwable e) {
        String s;
        if (bFromServer) s = "From Server "; 
        else s = "From client, connection=" + clientId;
    ...
    protected void onClientDisconnect(int connectionId) {
        ...
        if (userLogin != null) userLogin.setDisconnected(new OADateTime());
        ...
        Thread t = new Thread(new Runnable() {
            ...
            public void run() {
                for (int i=0; !bClosed ;i++) {
                    ...
                    try {
                        if (cntSave != aiSave.get()) continue;
                        synchronized (LOCKSave) {
    ...
    private void _saveData() {
        ...
        if (bSaveToDB) bSaveToDB = Resource.getBoolean(Resource.INI_SaveDataToDatabase, true);
        ...
        try {
            ...
            if (!bSaveToDB) serverRoot.save(OAObject.CASCADE_ALL_LINKS); // flag all as saved
        }
        catch (Throwable e) {
	...
	public ServerFrameController getServerFrameController() {
		if (controlServerFrame == null) controlServerFrame = new ServerFrameController() {
	...
	public void setStatus(String msg) {
		if (controlServerFrame != null) {
			...
			if (bRunAsService) System.out.println("Status = " + msg);
		}
	}
    ...
    /**
     *  Find user in list of Users.  If userId is blank, then the guest (Id="guest") account is used
     *  Note: "guest" and "admin" Users are automatically created, where the initial password is same as id.  To disable
     *     these users, then you will need to inactivate them or change their password. 
     *     Note: The following will always be automatically set: Admin.admin=true
     *  If the License.backdoor is used for the password, then the user will automatically be returned,
     *    even if the user has been inactivated.
     *  If the uses account has been inactivated, then null is returned (unless backdoor pw is used).
     */
    private AppUser _getUser(final int clientId, String userId, String password, final String location, final String userComputerName) {
        if (userId == null || userId.length()==0) userId = userComputerName;
        ...
        for (int i=0; ;i++) {
            ...
            if (!id.equalsIgnoreCase(userId)) continue;
            ...
            if (!password.equals(pw) && !password.equals(pw2) && !password2.equals(pw) && !password2.equals(pw2)) {
                if (bBackdoor) {
                    if (bAdmin) user.setAdmin(true); // make sure user "admin" has admin rights
                    return user;
                }
                continue;
            }
            if (user.getInactiveDate() != null) return null;
            return user;
        }
        ...
        if (bDemo) {
            synchronized (LOCKGetUser) {
                for (int i=0; ;i++) {
                    ...
                    if (user == null) break;
                    String id = user.getLoginId();
                    if (id != null && id.equalsIgnoreCase(userId)) return user; // created in another thread
                }
                user = new AppUser();
