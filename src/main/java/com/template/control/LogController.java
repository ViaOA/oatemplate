...
public class LogController {
	...
	public LogController(boolean bServer) {
        ...
        if (bServer) strDateTimeStamp = dtStarted.toString("yyyyMMdd");
        else strDateTimeStamp  = dtStarted.toString("yyyyMMdd_HHmm");
        ...
        */
        ...
        if (bServer) {
            ...
            s = "com.viaoa.ds.jdbc.delegate.DBLogDelegate";        
            log = Logger.getLogger(s);
            alLogger.add(log);
            log.setLevel(Level.FINE);
	...
	private FileHandler createFileHandler(final String prefix, final Level level, boolean bCreateMax) {
		try {
            ...
            if (bCreateMax) fileName = createLogFileName(prefix + "_%g");
            else fileName = createLogFileName(prefix);
    		...
    		if (bCreateMax) {
    			fileHandler = new FileHandler(fileName, 5 * 1024 * 1024, 5, true) {
                    ...
                    public synchronized void publish(LogRecord record) {
                        ...
                        if (handler == null) super.publish(record);
                        else handler.publish(record);
                    }
                };
    		}
    		else {
	...
	public Level getLevel(String name) {
        if (name == null) return Level.OFF;
    	if (name.equalsIgnoreCase("FINEST")) return Level.FINEST;
    	if (name.equalsIgnoreCase("FINER")) return Level.FINER;
    	if (name.equalsIgnoreCase("FINE")) return Level.FINE;
    	if (name.equalsIgnoreCase("CONFIG")) return Level.CONFIG;
    	if (name.equalsIgnoreCase("INFO")) return Level.INFO;
    	if (name.equalsIgnoreCase("WARNING")) return Level.WARNING;
    	if (name.equalsIgnoreCase("SEVERE")) return Level.SEVERE;
        return Level.OFF;
	}
    ...
    */
    ...
    protected void _fastLog2(FastLog fl) throws Exception {
        ...
        OADateTime dt = fl.dt;
        if (dt == null) {
            if (fl.ts != 0) dt = new OADateTime(fl.ts);
            else dt = new OADateTime();
        }
