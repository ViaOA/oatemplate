...
import com.template.view.client.*;
...
public abstract class ClientFrameController {
    ...
    /***
    protected void onShowServerInfoMessage() {
        ServerInfo si = ModelDelegate.getServerInfo();
        if (si != null && si.getSendMessageDateTime() != null) {
            String msg = si.getSendMessage();
            if (!OAString.isEmpty(msg)) {
                OADateTime dt = si.getSendMessageDateTime();
                msg = "<b style='color:gray'>" + dt.toString() + "</br><br>" + msg;
                getFrame().setNotifyBalloon("<html>" + msg);
            }
        }
    }

    protected void onShowServerStatus() {
        ServerInfo si = ModelDelegate.getServerInfo();
        if (si != null) {
            String msg = si.getSendMessage();
            if (OAString.isEmpty(msg)) msg = "";
            else {
                OADateTime dt = si.getSendMessageDateTime();
                if (dt != null) {
                    msg = "<b style='color:gray'>" + dt.toString() + "</br><br>" + msg;
            ...
            msg = "<b><u>Server is running</u></b><br>" + msg;
            getFrame().setNotifyBalloon("<html>" + msg);
        }
    }
    */
    ...
    public ClientFrame getFrame() {
                ...
                void onExit() {
                    ...
                    if (Resource.getBoolean(Resource.INI_AutoLogout)) {
                        ...
                        i = JOptionPane.showConfirmDialog(getFrame(), "Ok to close " + Resource.getRunTimeName(),
                                "Exit " + Resource.getValue(Resource.APP_ApplicationName), JOptionPane.YES_NO_OPTION,
        ...
        try {
            ...
            if (x >= 0 && y >= 0 && w > 100 && h > 100) {
                ...
                if (bounds.width > dimScreen.width) bounds.width = 0; 
                if (bounds.height > dimScreen.height) bounds.height = 0; 
                ...
                if (ext != JFrame.MAXIMIZED_BOTH) ext = JFrame.NORMAL;
            }
    ...
    public void close() {
        ...
        Resource.setValue(Resource.TYPE_Client, Frame_X, p.x + "");
        Resource.setValue(Resource.TYPE_Client, Frame_Y, p.y + "");
        Dimension d = frm.getSize();
        Resource.setValue(Resource.TYPE_Client, Frame_Width, d.width + "");
        Resource.setValue(Resource.TYPE_Client, Frame_Height, d.height + "");
        Resource.setValue(Resource.TYPE_Client, Frame_Extended, frm.getExtendedState() + "");
        Resource.save();
    ...
    public void onSaveAsPdf() {
        LOG.fine("called");
        if (getPdfController() == null || controlPrint == null) return;
    ...
    protected void refreshReport(final OAHTMLReport rpt, final boolean bShowStatus) {
        if (bShowStatus) setProcessing(true, "Preparing report ...");
        SwingWorker<String, Integer> sw = new SwingWorker<String, Integer>() {
            ...
            @Override
            protected void done() {
                if (bShowStatus) setProcessing(false);
    ...
    public PrintController getPrintController() {
        if (controlPrint == null) {
            ...
            if (frm != null) controlPrint.setParentWindow(frm);
    ...
    public ReportController getReportController() {
        if (controlReport != null) return controlReport;
