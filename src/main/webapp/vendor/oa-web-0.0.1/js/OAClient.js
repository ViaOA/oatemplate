...
this.mapElementToDataName.set(ele, this.mapElementToDataName.get(eleParent) + '.' + dataNamePath);
    ...
    getServerTemplate: async function(url, templateName) {
        try {
    ...
    loadTemplateFromServer: async function(ele, url, templateName) {
        let html = await this.getServerTemplate(url, templateName);
        ele.innerHTML = html;
    },
    
    async createNewSessionOnServer() {
        const url = `/jsp/oa-web-app.jsp`;
        this.getJavaScriptFromServer(url);
    },
    
    getJavaScriptFromServer: async function(url) {
        fetch(url)
            .then(response => {
                if (!response.ok) {
                    throw new Error(`response for getJavaScriptFromServer ${url} was not OK`);
                }
